function Jacobi(a,b,c,d,n,x)                                                %% Define the function
...
while k > 1                                                                 %% Create an infinite loop
    ...
    for i = 2:n-1
        x(i,k) = (1/b(i)) * (d(i)-a(i-1)*x(i-1,k-1)-c(i)*x(i+1,k-1));       %% Iterate from 2 to n-1
    end
    x(n,k) = (1/b(n)) * (d(n)-a(n-1)*x(n-1,k-1));                           %% Iterate element n
...
disp(horzcat('The maximum difference between the iterations', char(10), 'by the Jacobi method is: ',num2str(abs(max(x(:,k)-x(:,k-1))))))
